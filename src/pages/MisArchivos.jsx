...
export default function MisArchivos() {
  ...
  const [loadingArchivosInscriptos, setLoadingArchivosInscriptos] =
    useState(false); // Estado para la carga de archivos inscriptos
  ...
  const handleVerArchivo = async (fileId, fileType) => {
    ...
    try {
      if (fileType.includes("pdf") || fileType.includes("image")) {
        ...
        setPreviewError(
          "Este tipo de archivo no tiene una vista previa disponible."
        );
      }
    } catch (err) {
      console.error("Error al abrir el archivo:", err);
      setPreviewError(
        "No se pudo cargar la vista previa del archivo. Asegúrate de que el formato sea compatible (PDF, imágenes, Word, PowerPoint) o verifica tu conexión."
      );
  ...
  const closePreviewModal = () => {
    ...
    if (
      previewUrl &&
      (fileTypeToPreview?.includes("pdf") ||
        fileTypeToPreview?.includes("image"))
    ) {
      ...
      <div className="mis-archivos-container">
        <div className="header">
          ...
          >
            ...
            >
              Ingresa tu Gmail para ver tus archivos:
            </label>
            <input
              id="gmail-input"
              type="email"
            ...
            >
              <span className="btn-text">
                {loadingArchivosInscriptos ? "Buscando..." : "Ver mis archivos"}
              </span>
...
`}</style>
        ...
        <div className="archivos-section-content">
          {" "}
          {/* Nuevo div para contener las dos posibles secciones de archivos */}
          {porMi === true ? (
                  ...
                  {archivos.length === 0 ? (
                    buscado && !error ? ( // Solo mostrar este mensaje si ya se buscó y no hay error
                      <div className="no-archivos">
                        No hay archivos disponibles para este Gmail.
                      </div>
                    ) : (
                      // Mensaje inicial si no se ha buscado todavía
                      <div className="no-archivos">
                        Ingresa tu Gmail para buscar archivos.
                      </div>
                    )
                  ) : (
                    archivos
                          ...
                          <div className="archivo-actions">
                            ...
                            <button
                              onClick={() =>
                                handleVerArchivo(
                                  presentacion._id,
                                  presentacion.fileType
                                )
                              }
                              className="event-file-download-pro"
                              disabled={
                                isPreviewLoading &&
                                fileTypeToPreview === presentacion.fileType
                              } // Deshabilitar solo el botón que se está cargando
